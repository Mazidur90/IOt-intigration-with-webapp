...
df["kmeans_detected"] = df["kmeans_anomaly"]
...
df['hover_text'] = df.apply(lambda row:
    ...
    f"KMeans Dist: {row.get('kmeans_distance', 0):.2f}", axis=1)

# === Evaluation Labels ===
def get_eval_type(row):
    if row["is_anomaly"] == 1 and row["kmeans_anomaly"] == 1:
        return "True Positive"
    elif row["is_anomaly"] == 1 and row["kmeans_anomaly"] == 0:
        return "False Negative"
    elif row["is_anomaly"] == 0 and row["kmeans_anomaly"] == 1:
...
fig = px.scatter(
    ...
    title="📊 KMeans Detection PCA — TP, FN, FP, TN Visualized"
)
...
fig_conf.update_layout(title="📊 KMeans Confusion Bar", yaxis_title="Count")

# === Table: Only Detected Anomalies ===
df_kmeans_anomalies = df[df["kmeans_anomaly"] == 1].copy()
table_columns = FEATURES + ["kmeans_distance", "evaluation_type"]
table_data = df_kmeans_anomalies[table_columns].round(2).to_dict('records')

# === Table: FP & FN Breakdown ===
df_eval_table = df[df["evaluation_type"].isin(["False Negative", "False Positive"])].copy()
eval_table_columns = FEATURES + ["kmeans_anomaly", "is_anomaly", "evaluation_type"]
eval_table_data = df_eval_table[eval_table_columns].round(2).to_dict("records")

# === DASH APP ===
app = Dash(__name__)
app.title = "KMeans Dashboard"

app.layout = html.Div([
    html.H2("🌿 KMeans Anomaly Detection Dashboard", style={"textAlign": "center", "color": "white"}),

    dcc.Graph(figure=fig),

    html.H3("📋 Detected by KMeans", style={"color": "white"}),
    ...
    html.Div([
        html.H4("📈 KMeans Evaluation Metrics", style={"color": "white"}),
